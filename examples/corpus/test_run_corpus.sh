...
default_dir="$ROOT/default-bound"
mkdir -p "$default_dir"
printf 'id\tkind\tdata\texpected\ndefault-id\tg2\t[]\ttruth\n' \
    > "$default_dir/input.tsv"
: > "$default_dir/record"
run_target "default bound" ok "$default_dir/record" "$default_dir/run.log" \
    -i "$default_dir/input.tsv" -o "$default_dir/output.tsv" -j 1
assert_contents "default bound" "$default_dir/output.tsv" \
    "${header}"$'default-id\tg2\t1024\ttruth\tcomputed\t0.01\tok\n'

unsound_dir="$ROOT/unsound"
mkdir -p "$unsound_dir/work"
printf 'id\tkind\tdata\texpected\nunsound-id\tg2\t[]\ttruth\n' > "$unsound_dir/input.tsv"
printf 'unsound-id\tg2\t64\ttruth\tbound\t0.01\tunsound\n' \
...
run_target "unsound harvest" ok "$unsound_dir/record" "$unsound_dir/run.log" \
    ...
    -w "$unsound_dir/work" -B 64 -j 1
assert_contents "unsound harvest" "$unsound_dir/output.tsv" \
    "${header}"$'unsound-id\tg2\t64\ttruth\tbound\t0.01\tunsound\n'
...
run_target "initial error" error "$retry_dir/record" "$retry_dir/first.log" \
    -i "$retry_dir/input.tsv" -o "$retry_dir/output.tsv" -B 64 -j 1
assert_contents "initial error" "$retry_dir/output.tsv" \
    "${header}"$'retry-id\tg2\t64\ttruth\tnone\t0.01\terror\n'
...
assert_contents "successful retry precedence" "$retry_dir/output.tsv" \
    "${header}"$'retry-id\tg2\t64\ttruth\tcomputed\t0.01\tok\n'
...
printf 'victim\tg2\t64\ttruth\tbound\t0.01\tok\n' \
...
run_target "part boundary" ok "$boundary_dir/record" "$boundary_dir/run.log" \
    ...
    -w "$boundary_dir/work" -B 64 -j 1
assert_contents "part boundary" "$boundary_dir/output.tsv" \
    "${header}"$'victim\tg2\t64\ttruth\tbound\t0.01\tok\n'
...
RUN_CORPUS_FAKE_MAGMA=1 FAKE_STATUS=ok FAKE_RECORD="$guard_dir/record" \
    ...
    -o "$guard_dir/work/output.tsv" -w "$guard_dir/work" -B 64 -j 1 \
...
# 529.a1 is sharp at B = 8 with [RR, RR]; HH cannot embed in that bound.
...
ENDO_SPEC="$HERE/../../endomorphisms/magma/spec" \
    ...
    -w "$real_dir/work" -B 8 -j 1 -c 2 > "$real_dir/run.log" 2>&1 || real_rc=$?
