...
for iotaK in InfinitePlacesExtra(K) do
    ...
    if Abs(genKCC - genLCC) lt CC`epscomp then return iotaK; end if;
end for;
...
intrinsic NumberFieldExtra(aCCs::SeqEnum[FldComElt], K::Fld : UpperBound := 16, DegreeDivides := Infinity()) -> Fld, SeqEnum
...
tupsa := [ ];
for aCC in aCCs do
    Lnew, tupsa, hnew := ExtendNumberFieldExtraStep(L, tupsa, aCC : UpperBound := UpperBound, DegreeDivides := DegreeDivides);
    if Lnew ne L then
        h := h*hnew;
        vprint EndoFind, 2 : "";
...
function ExtendNumberFieldExtraStep(K, tupsa, anewCC : UpperBound := 16, DegreeDivides := Infinity())
...
intrinsic SplittingFieldExtra(aCCs::SeqEnum[FldComElt], K::Fld : UpperBound := 16, DegreeDivides := Infinity()) -> Fld, SeqEnum
...
tupsa := [ ];
for aCC in aCCs do
    Lnew, tupsa, hnew := ExtendSplittingFieldExtraStep(L, tupsa, aCC : UpperBound := UpperBound, DegreeDivides := DegreeDivides);
    if Lnew ne L then
        h := h*hnew;
        vprint EndoFind, 2 : "";
        vprint EndoFind, 2 : "Number field extended. Current field:";
...
function ExtendSplittingFieldExtraStep(K, tupsa, anewCC : UpperBound := 16, DegreeDivides := Infinity())
...
if IsQQ(K`base) then
    return ExtendSplittingFieldExtraStepQQ(K, tupsa, anewCC : UpperBound := UpperBound, DegreeDivides := DegreeDivides);
end if;
return ExtendSplittingFieldExtraStepGen(K, tupsa, anewCC : UpperBound := UpperBound, DegreeDivides := DegreeDivides);

end function;


function ExtendSplittingFieldExtraStepQQ(K, tupsa, anewCC : UpperBound := 16, DegreeDivides := Infinity())
// Let K be a SplittingFieldExtra with elements tupsa, and let anewCC be a
// complex number. This function returns the splitting field generated by K and
// anewCC, and transports tupsa to that field. Keeps track of morphisms.
...
function ExtendSplittingFieldExtraStepGen(K, tupsa, anewCC : UpperBound := 16, DegreeDivides := Infinity())
// Let K be a SplittingFieldExtra with elements tupsa, and let anewCC be a
// complex number. This function returns the splitting field generated by K and
// anewCC, and transports tupsa to that field. Keeps track of morphisms.
...
assert K eq Domain(h); assert L eq Codomain(h);
if IsQQ(K) then return K ! a; end if;
return K ! (a @@ h);
...
if not Compat then return M, hKM, hLM; end if;
