...
{Given complex number aCCs and a NumberFieldExtra F, finds the splitting field generated by the elements of aCCs and realizes said elements in that field. An inclusion map is also returned.}

if #aCCs eq 0 then
    return F, [ ], CanonicalInclusionMap(F, F);
end if;

/* Initiate and make sure that the field of K is good for comparison purposes */
K := F; h := CanonicalInclusionMap(K, K);
...
tupsa := [ ];
for aCC in aCCs do
    Knew, tupsa, hnew := ExtendSplittingFieldExtraStep(K, tupsa, aCC);
    if Knew ne K then
        h := h*hnew;
        vprint EndoFind, 2 : "";
        vprint EndoFind, 2 : "Number field extended. Current field:";
        vprint EndoFind, 2 : Knew;
    end if;
    K := Knew;
end for;
...
return K, [ tupa[1] : tupa in tupsa ], h;
...
for aCC in aCCs do
    Lnew, tupsanew, hnew := ExtendNumberFieldExtraStep(L, tupsanew, aCC);
    if Lnew ne L then
        h := h*hnew;
    end if;
    L := Lnew;
